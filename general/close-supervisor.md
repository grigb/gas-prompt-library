...
- loaded prompt or charter under
  `~/.agents-gas-prompt-library/agents/agent-blocker-supervisor/` or
  `~/.agents/agents/blocker-engineer/SUPERVISOR.md`;
- blocker files, blocker indexes, supervisor runstate/status, or blocker
  lifecycle work under `~/.agents/agents/blocker-engineer/`;
...
- Project blocker indexes and
  `~/.agents/.dev/ai/blockers/MASTER-INDEX.md` are summary surfaces.
...
`~/.agents/prompts/creation/CREATE-SESSION-RECORD.md`, which also
...
may be using right now: `~/.agents/.dev/ai/blockers/` including
...
`~/.agents/.dev/ai/blockers/INDEX.md`, reread
`~/.agents/docs/protocols/woq-role-lifecycle.md` and use
...
- `~/.agents/agents/blocker-engineer/ms-dispatch.md`.
...
SUPERVISOR CLOSE CAPTURE PACKET:
  ...
  supervisor subroutine path: ~/.agents/prompts/general/close-supervisor.md
  Session record entrypoint: ~/.agents/prompts/creation/CREATE-SESSION-RECORD.md
  ...
  Master index sync:
    - path: ~/.agents/.dev/ai/blockers/MASTER-INDEX.md
