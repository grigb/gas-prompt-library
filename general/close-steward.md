...
`~/.agents/prompts/creation/CREATE-SESSION-RECORD.md` so there is one
...
- Master Steward overlay, Master Steward startup overlay, or master-steward
  private paths under `~/.agents-private/project-steward/master-steward/`;
...
`~/.agents/prompts/creation/CREATE-SESSION-RECORD.md`. Before asserting
...
- `steward_subroutine_path`:
  `~/.agents/prompts/general/close-steward.md`;
...
Do not use `~/.agents/prompts/general/close-steward.md` as
...
**not** hand-maintained. `~/.agents/.dev/ai/workorders/WO-INDEX.md` is
...
`~/.agents/.venv/bin/python3 -m tools.woq.cli work-order write`, and
the index is rebuilt from it. Owner-approved cutover 2026-08-12,
WO-GAS-WOQLIVE-014; details in
`~/.agents/docs/protocols/woq-role-lifecycle.md`.
...
`~/.agents/agents/tuning/steward-tuning-log.md`. Do NOT fix your own
...
`~/.agents/docs/overviews/MASTER-STEWARD-VARIANT.md#concurrent-closeout-and-shared-home-preservation`.
...
`~/.agents-private/project-steward/master-steward/` indexes,
...
`~/.agents/agents/blocker-engineer/ms-updates.md` with the provenance
required above.
...
write to `~/.agents/agents/blocker-engineer/ms-dispatch.md` with the
...
`~/.agents-private/project-steward/master-steward/inbox/` with the
provenance required above.
...
STEWARD CLOSE CAPTURE PACKET:
  ...
  steward_subroutine_path: ~/.agents/prompts/general/close-steward.md
...
Then return to
`~/.agents/prompts/creation/CREATE-SESSION-RECORD.md` for the final
