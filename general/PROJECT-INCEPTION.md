...
run `~/.agents/scripts/register-project.sh`. If the owner names an
...
```bash
~/.agents/scripts/register-project.sh "<project-root>" --name "<display name>"
```
...
`~/.agents/scripts/check-agents-freshness.sh "<project-root>"`, and
...
Initiative incepted: {concept name}
  ...
  Team: {roster summary}
  Registration: {registered with ~/.agents/scripts/register-project.sh OR gated until project root exists}
  Board direction: {absolute path}
