...
Converting an artifact to a WO affects two status surfaces: the new WO file and
`.dev/ai/workorders/WO-INDEX.md`. The converter may create the WO file when
operating in an authorized intake/conversion role. Index synchronization must
use the current safe path:

- GAS root `WO-INDEX.md`: WOQ shared-status safe writer with the current full
  target hash. On refusal, write the proposed index text and refusal details to
  the result artifact for parent/session-owned assimilation.
...
- Workers, QA, and read-only agents default to result-artifact-only
  ...
  change and the `WO-INDEX.md` synchronization required through the authorized
  role boundary.

Exception: if the converted WO belongs to an exact owner-approved generated
boundary listed in `/Users/grig/.agents/docs/protocols/woq-role-lifecycle.md`
(currently `woq-live-status`, or `WO-GASECAP-20260714-001` through `006` in
`gas-external-capability-integration`), do not manually add or propose its
WO-INDEX entry. The provenance-marked section is generated by
`woq_shadow_sync` from WOQ + WO files. All unflipped workstreams retain the
safe paths above.
...
After saving the WO file, synchronize the WO file status and
`.dev/ai/workorders/WO-INDEX.md` entry through the allowed role boundary. Use
the GAS shared-status safe writer for the GAS root index, the project-local
`.WO-INDEX.lock/` protocol for project indexes, or write the proposed WO file
status and index update to the result artifact if this agent lacks authority or
the safe writer/lock refuses the update.
