...
In the GAS root, converting an artifact to a WO affects exactly one status
surface: the new WO file. The GAS root work-order index is generated from WOQ
and is **not** hand-maintained —
`~/.agents/.dev/ai/workorders/WO-INDEX.md` is retired and the index is
`~/.agents/.dev/ai/workorders/WO-INDEX.woq-generated-view.md`.
Hand-writes to it are refused. Do not add, queue, or propose an index entry for
it; write the WO file (`woq work-order write`) and the index is rebuilt from it.
Owner-approved cutover 2026-08-12, WO-GAS-WOQLIVE-014. This supersedes the
narrow generated-section boundaries (`woq-live-status`,
`gas-external-capability-integration`, `global-work-order-q-system`) that once
applied inside that index.

In every other Project, conversion affects two status surfaces: the new WO file
and `{PROJECT_ROOT}/.dev/ai/workorders/WO-INDEX.md`. The converter may create
the WO file when operating in an authorized intake/conversion role. Index
synchronization must use the current safe path:

- Project-local `WO-INDEX.md`: acquire `.WO-INDEX.lock/`, reread the index
  after lock acquisition, write only the scoped new-entry/status change, and
  fall back to `index-pending/<role>/` or the result artifact on contention.
- Workers, QA, and read-only agents default to result-artifact-only
  ...
  change and the project-local `WO-INDEX.md` synchronization required through
  the authorized role boundary.
...
# GAS root: the index is generated - read the generated view, not the tombstone
cat .dev/ai/workorders/WO-INDEX.woq-generated-view.md 2>/dev/null \
...
Apply the canonical WO-authoring gate policy in
`~/.agents/docs/standards/WO-FORMAT-STANDARD.md#wo-authoring-gate-policy`.
The converted WO should be executable by default. Do not carry forward routine
"ask the owner", review, or approval language from stale handoffs/session
records unless the owner requested that gate or the current verification proves
a real missing-info/access, destructive, production-data-loss,
legal/financial/business-authority, scope-expansion, or unrecommendable
product/strategy ambiguity gate. Acceptance criteria, QA, verification, and
...
### Step 3: Synchronize WO File and Index

In the GAS root, the WO file is the only write — its index is generated from
WOQ, hand-writes are refused, and no index entry is added or proposed.

In every other Project, synchronize the WO file status and the project-local
`.dev/ai/workorders/WO-INDEX.md` entry through the allowed role boundary: use
the `.WO-INDEX.lock/` protocol, or write the proposed WO file status and index
update to the result artifact if this agent lacks authority or the lock refuses
the update.

### Step 4: Track in Project System
...
### Step 6: Archive Legacy Handoff (if fully converted)
```bash
...
| > 7 days | Validate current state before converting; ask the owner only if required facts cannot be verified |
| > 30 days | Recommend archival unless current evidence proves remaining work is still valid; ask the owner only if required facts cannot be verified |
...
**Recommended Action:** Verify current state before conversion. Ask the owner
only if the required facts cannot be established from current project evidence.
...
**Options:**
1. I will verify current state and convert only remaining valid work
