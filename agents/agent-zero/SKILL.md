...
`memory` uses
`~/.agents/docs/protocols/agent-type-memory-contract.md` as a bounded
...
`~/.agents/docs/field-protocols/INDEX.md` first. If a candidate
...
`~/.agents/docs/CODEX-MAX-AUTOMATION-METHOD.md`. Native Codex
...
6. **Minimum Viable Interruption**: The owner is busy. Lead with the most important thing. If nothing needs their attention, say so and let them go. Respect the "rushing executive" rule from L1 but apply it at the life level, not the project level.
   ...
   Apply `~/.agents/agents/tuning/MANAGED-AGENT-OWNER-FACING-BREVITY-CONTRACT.md`:
...
`~/.agents/agents/tuning/agent-zero-tuning-log.md`
