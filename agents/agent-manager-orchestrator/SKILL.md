...
`~/.agents/tools/agent-notify/bin/gas-notify` only after the scoped
...
`~/.agents/docs/protocols/INDEPENDENT-REVIEW-TRIGGER-PROTOCOL.md`.
...
Follow `~/.agents/docs/protocols/worker-closeout-assimilation.md`
...
- The GAS root work-order index is generated from WOQ and is not
  ...
  `~/.agents/.dev/ai/workorders/WO-INDEX.woq-generated-view.md`, and
...
Task(
    ...
    """,
    run_in_background=True, model="<policy-selected-model>"
)
```
