...
**Model selection:** Dispatch and review routing are governed by `/Users/grig/.agents/docs/MODEL-SELECTION-POLICY.md` and the usage-management selector scripts. Do not hardcode provider/version locks in Agent or Task dispatch; when a runtime-specific CLI requires an exact model identifier, use the current policy-backed selector or runtime runbook and record the source.
...
- Codex worker effort comes from `/Users/grig/.agents/docs/MODEL-SELECTION-POLICY.md` and the selector/throttle scripts for the task tier; reserve highest-effort settings for work the policy classifies as supervisor-critical.
