...
`~/.agents/docs/methodologies/project-state-sync-method.md`
Connector catalog and contract:
`~/.agents/prompts/agents/agent-project-state-sync/references/connectors.md`
...
1. Locate the project PSS config (default
   ...
   `~/.agents/prompts/agents/agent-project-state-sync/config-template.yaml`
...
3. Note the current watermarks:
   `~/.agents/.venv/bin/python3 {SKILL_DIR}/scripts/pss_ingest.py show --config {CONFIG}`
...
```
~/.agents/.venv/bin/python3 \
  ~/.agents/prompts/agents/agent-project-state-sync/scripts/pss_ingest.py \
  plan --config {CONFIG}
```
...
`~/.agents-gas-prompt-library/meaning-extraction/execution-decisions-work-order-extraction-prompt.md`.
...
- WO + INDEX sync: create/update WO drafts to WOs per policy and keep WO files
  ...
  root is `~/.agents`, do not touch the index at all: the GAS root
...
- Cascade terminal completions: for every WO or blocker moved terminal in this
  ...
  `~/.agents/.venv/bin/python3 ~/.agents/scripts/reconcile-dependents.py <id>`
