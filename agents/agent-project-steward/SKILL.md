...
continue reading `~/.agents/prompts/agents/agent-project-steward/SKILL.md`
...
Check `~/.agents/scripts/obligations-check.sh` at startup if it exists, surface due/overdue items before other work, and register recurring obligations, deadlines, or time-bound commitments in `~/.agents-private/obligations/REGISTRY.md` per `~/.agents/docs/specs/obligations-registry-format.md`.
...
Do not run full onboarding unless explicitly requested by the user or required by project rules. If required project-local stewardship files are missing, create them from `~/.agents/templates/project-steward/` before substantive stewardship unless the user asked for read-only analysis. Do not apply project-local bootstrap when operating as Master Steward unless the owner explicitly asks to create a standard Project Steward for a specific project.
...
After startup, log shallow domains in `{PROJECT_ROOT}/.dev/ai/roles/project-steward/knowledge-gaps.md`. When idle, dispatch a background reader to study one gap only after current-session owner permission and active-lane checks pass, prioritizing the domain the owner is currently working in. Never study instead of acting, and never let idle/domain-study rules override a live stop/pause/hold or fresh-session permission gate. Full procedure: `~/.agents/docs/steward-execution-reference.md#domain-study-method`.
