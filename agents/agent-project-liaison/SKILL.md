...
Use `~/.agents/templates/project-liaison/` when bootstrapping a new
...
2. Run the bootstrap tool to register this session and request the active primary lease:
   `python3 .dev/scripts/project_liaison_bootstrap.py --harness <harness_name> --request-primary --owner-approved-by "Owner"`
...
4. **Heartbeat Loop**: To prevent the interactive lease from expiring (default 1 hour TTL) while the harness session remains open, you MUST immediately schedule a 30-minute heartbeat:
   ...
   - When the heartbeat notification fires, run the renew CLI command to refresh the TTL:
     `~/.agents/.venv/bin/python3 .dev/scripts/project_liaison_runtime.py renew --project-root {PROJECT_ROOT} --runner-id {RUNNER_ID} --ttl-seconds 3600`
...
`~/.agents/docs/OWNER-FACING-BRIEF-STANDARD.md` and
`~/.agents/docs/OWNER-FACING-BRIEF-STYLE-GUIDE.md`; use
`~/.agents/templates/BRIEF-TEMPLATE.md` as the concrete shape when a
...
`~/.agents/scripts/get-filename-prefix.sh` and a short kebab-case
...
1. **Queue Root Location:** The active queue root folder for this runtime is `~/.agents/data/conversation-directory/relay-artifacts`. When invoking `dc-relay-apply` or other queue tooling, always pass this custom `--queue-root` path explicitly.
...
`~/.agents/style-guides/writing/OWNER-FACING-AGENT-MESSAGE-STYLE-GUIDE.md` and
`~/.agents/agents/tuning/MANAGED-AGENT-OWNER-FACING-BREVITY-CONTRACT.md`.
