...
```text
~/.agents/plugin-packages/<plugin>/
  .codex-plugin/plugin.json
  skills/<frontmatter-name>/SKILL.md
```

`~/.agents/config/plugin-bundles.json` maps canonical source,
frontmatter identity, plugin target, policy, exclusions, and provenance. Run
`~/.agents/tools/gas-plugin-packager/generate.py` to update generated
...
metadata:
  ...
  # models using ~/.agents/docs/MODEL-SELECTION-POLICY.md and
  # ~/.agents/tools/usage-management/scripts/select-model.sh.
