...
`~/.agents/agents/assistant/memory/`; review candidate memories only
...
1. Read `~/.agents/agents/assistant/memory/README.md`.
...
3. For an explicit Personal Assistant activation, onboarding, or re-entry, read
   `~/.agents-private/assistant/README.md` only if it exists, then at
