...
1. Ingest pending cards via `~/.agents/tools/aeos/decision_queue.py get-pending`.
2. Speak each card's context, options, and recommendation tersely and clearly using `~/.agents/tools/aeos/voice_facilitator.py`.
3. Capture the owner's choice (`go`, option number, or spoken dictate).
4. Record the answer and immediately stage a resume packet via `~/.agents/tools/aeos/relay_dispatcher.py` to notify the waiting session.
5. Ingest completed answers into the heuristic memory via `~/.agents/tools/aeos/decision_learner.py ingest`.
...
```bash
~/.agents/tools/aeos/voice_facilitator.py
```
