...
follow `~/.agents/docs/protocols/INDEPENDENT-REVIEW-TRIGGER-PROTOCOL.md`.
...
`memory` uses
`~/.agents/docs/protocols/agent-type-memory-contract.md`; review
...
`~/.agents/.venv/bin/python3 -m tools.woq.cli project-index write`
...
- For the lifecycle status of an exact Work Order, query the selected-portfolio
  read boundary first:
  `~/.agents/.venv/bin/python3 -m tools.woq.cli portfolio-status --manifest ~/.agents/config/woq-authority-boundaries/woq-selected-portfolio-lifecycle-read-2026-07-19.json --project-root {project_path} --work-order-id {WO_ID}`.
...
- The GAS root work-order index is generated from WOQ and is **not**
  hand-maintained. `~/.agents/.dev/ai/workorders/WO-INDEX.md` is
  retired; the index is
  `~/.agents/.dev/ai/workorders/WO-INDEX.woq-generated-view.md`, read
  only. Hand-writes to it are refused — both `woq shared-status write` and
  `woq project-index write` reject that target. Do not update the index, do not
  stage a proposed index transition for it, and do not queue `index-pending`
  work against it. Update the Work Order file only
  (`~/.agents/.venv/bin/python3 -m tools.woq.cli work-order write`)
...
for those sections. As of the 2026-08-12 cutover the entire GAS root index is
generated this way. The write rules above remain mandatory for unflipped
...
`~/.agents/agents/global-triage/` for executable WOs; that directory
...
```bash
read -r MODEL EFFORT < <(~/.agents/tools/usage-management/scripts/select-model.sh "$TIER" --provider claude)
...
**Invocation (CLI):**
```bash
read -r MODEL _ < <(~/.agents/tools/usage-management/scripts/select-model.sh "$TIER")
...
`~/.agents/docs/SUB-AGENT-ORCHESTRATION-GUIDE.md#pre-dispatch-worker-packet-sizing`.
...
`~/.agents/docs/protocols/worker-closeout-assimilation.md`.
...
Shared-Index Contract. The GAS root index is generated — there is nothing to
assimilate into it; update the Work Order file with `woq work-order write` and
the index is rebuilt from it. For project-local `WO-INDEX.md`, use
`woq project-index write` and record the helper-created pending artifact on
contention.
...
- Local document-only teams (MW-1 shadow/hardening only until owner cutover):
  `~/.agents/tools/teams/bin/teams`
- Inter-agent communication spec: `~/.agents/docs/INTER-AGENT-COMMUNICATION.md`
