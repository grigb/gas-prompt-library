...
`~/.agents/docs/MODEL-SELECTION-POLICY.md`. DC Relay batching and
...
bounded default of 3-High with normal verification.
...
`~/.agents/tools/usage-management/scripts/select-model.sh <1-5>` and use
...
`3-High` for bounded reasoning with normal verification (`4-Extra High` handles substantial uncertainty or the final-consequential floor; `5-Max` requires exceptional justification);
...
`~/work/obsidian-vault/distributed-creatives-vault/.dev/ai/processes/whatsapp/compact-relay-operating-card.md`
...
```bash
python3 ~/work/obsidian-vault/distributed-creatives-vault/.dev/ai/processes/whatsapp/scripts/dc_relay_lifecycle.py bump --compact
```

The helper reads the configurable threshold from
`~/work/obsidian-vault/distributed-creatives-vault/.dev/ai/processes/whatsapp/dc-relay-lifecycle-config.json`
and stores mutable counter state outside vault git history at
`~/.agents-private/whatsapp-live/relay-lifecycle/dc-vault.json`.
It may read
`~/.codex/automations/dc-relay-heartbeat/automation.toml` to observe
...
- Do not repair heartbeat lifecycle by editing raw files under
  `~/.codex/automations`; use the supported Codex app automation tool
...
6. If `rotation_handoff_required=true`, send a steward-visible lifecycle
   ...
   `~/work/obsidian-vault/distributed-creatives-vault/.dev/ai/processes/whatsapp/dc-relay-lifecycle-rotation-runbook.md`.
...
runbook: ~/work/obsidian-vault/distributed-creatives-vault/.dev/ai/processes/whatsapp/dc-relay-lifecycle-rotation-runbook.md
...
Read `~/.agents-gas-prompt-library/agents/agent-dc-relay/references/steward-result-contract.md`
