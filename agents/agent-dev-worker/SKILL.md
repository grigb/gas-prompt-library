...
`~/.agents/agents/global-triage/`. Read the linked global triage
record only if the WO itself is ambiguous or explicitly asks you to read it.

## WOQ Worker Lifecycle Contract

Follow `~/.agents/docs/protocols/woq-role-lifecycle.md` when the
...
`~/.agents/docs/protocols/woq-role-lifecycle.md`, capture the current
target hash with `~/.agents/.venv/bin/python3 -m tools.woq.cli shared-status hash`,
and write through `~/.agents/.venv/bin/python3 -m tools.woq.cli shared-status write`.
...
Canonical source: `~/.agents/docs/coding-rules/GENERAL-RULES.md#G18`.
...
`~/.agents/skills/CODING-SKILLS.md`, read every listed skill before
...
`~/.agents/skills/gas-codecraft/SKILL.md`. For implementation or
...
- run `~/.agents/.venv/bin/python3 ~/.agents/scripts/blocker-views-refresh.py --project <project_root>`
  or, if unavailable, `python3 ~/.agents/scripts/blocker-views-refresh.py --project <project_root>`;
...
> `{project}/.dev/ai/teams/` via `~/.agents/tools/teams/bin/teams`
