...
`~/.agents/agents/global-triage/` for work; that directory is
provenance and unresolved intake only.

## Execution

Canonical G18 source: `~/.agents/docs/coding-rules/GENERAL-RULES.md#G18`.
...
For `~/.agents/.dev/ai/PROJECT-STATUS.md`,
...
- run `~/.agents/.venv/bin/python3 ~/.agents/scripts/blocker-views-refresh.py --project {project_path}`
  or, if unavailable, `python3 ~/.agents/scripts/blocker-views-refresh.py --project {project_path}`;
