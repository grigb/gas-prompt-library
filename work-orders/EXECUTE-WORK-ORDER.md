...
Every status update must mention both:

- the WO file status/body change needed; and
- the `WO-INDEX.md` synchronization needed through the authorized parent,
  steward, liaison, triage, maintenance writer, GAS shared-status safe writer,
  or project-local lock lane.
...
`index-pending` or result-artifact fallback on lock contention.
...
`WO-INDEX.md` plus Work Order file. This read authority does not grant a worker
permission to write lifecycle state, claim a lease, dispatch, or execute work.

Generated-boundary exception: for an exact owner-approved generated boundary
listed in `/Users/grig/.agents/docs/protocols/woq-role-lifecycle.md`, perform the authorized WO-file
transition but do not write or propose the matching WO-INDEX change. The
provenance-marked section is regenerated by `woq_shadow_sync` from WOQ + WO
files. This exception applies to no other boundary.
...
6. **Status sync**: If explicitly authorized, update the WO file STATUS to
   ...
   exact `--result-artifact`, AGENT, and timestamp, then synchronize
   `WO-INDEX.md` through the safe writer or project-local lock lane. If not
   authorized, record both the WO file status change and required index
   synchronization in the result artifact.
...
3. Synchronize completion status through the allowed role boundary: update the
   ...
   work-order write` with a current full-file `--base-sha256`; synchronize
   `WO-INDEX.md` through the safe writer/project-local lock lane; otherwise
   record both proposed updates in the result artifact.
...
- If truly blocked, synchronize BLOCKED status through the allowed role
  ...
  proposed WO file update plus required `WO-INDEX.md` synchronization in the
  result artifact.
