...
`~/.agents/.venv/bin/python3 -m tools.woq.cli work-order write` with
...
hand-maintained: `~/.agents/.dev/ai/workorders/WO-INDEX.md` is retired
and the index is
...
`~/.agents/.dev/ai/workorders/`. The generated index states its own
...
`~/.agents/docs/protocols/woq-role-lifecycle.md`. This applies to
`~/.agents` only; every other Project's index keeps the rules above.
