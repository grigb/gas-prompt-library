...
`~/.agents/.dev/ai/PROJECT-STATUS.md`. Reread
...
1. **Update blocker INDEX files.** Ensure `{project_path}/.dev/ai/blockers/INDEX.md` reflects the current scan (already handled by catalog emission rules above). For `~/.agents/.dev/ai/blockers/INDEX.md`, use the WOQ safe writer with a current hash.
