...
The dual-write convention (canonical inside each project's `.dev/ai/deep-review/2026-05-23/`, mirror at `~/.agents/.dev/ai/deep-review/2026-05-23/<module>/`) is deliberate: the canonical copy lives where developers expect to find module-local audit history, the mirror keeps the entire program reviewable in one tree without requiring traversal across worktrees, and the global mirror is the synthesis surface that the holistic agent reads.
...
- Writes this framework to `~/.agents/prompts/security/deep-review-framework.md`.
- Writes the digest to `~/.agents/prompts/security/deep-review-framework-digest.md`.
...
- Produces `~/.agents/.dev/ai/deep-review/2026-05-23/00-holistic/cross-project-synthesis.md`.
...
- **Review-artifact access control.** Deep-review artifacts may contain quoted code with PII excerpts. The global mirror tree at `~/.agents/.dev/ai/deep-review/` should have access controls aligned with the sensitivity of the most-sensitive module. Document the access posture in the manifest if not yet defined.
...
2. Write `~/.agents/.dev/ai/deep-review/2026-05-23/URGENT-<MODULE>.md` containing exactly:
   ```
...
- Mirror: `~/.agents/.dev/ai/deep-review/2026-05-23/<module>/context.md`
...
- Mirror: `~/.agents/.dev/ai/deep-review/2026-05-23/<module>/review.md`
...
- Markdown ledger (human view): `~/.agents/.dev/ai/deep-review/2026-05-23/FINDINGS-LEDGER.md`
- JSONL ledger (authoritative): `~/.agents/.dev/ai/deep-review/2026-05-23/FINDINGS-LEDGER.jsonl`
...
Program date: 2026-05-23
Framework: ~/.agents/prompts/security/deep-review-framework.md
Framework version: 2.0
JSONL companion: ~/.agents/.dev/ai/deep-review/2026-05-23/FINDINGS-LEDGER.jsonl
...
`~/.agents/.dev/ai/deep-review/2026-05-23/FINDINGS-LEDGER.jsonl`
...
1. Create quarantine directory if it does not exist: `~/.agents/.dev/ai/deep-review/2026-05-23/_quarantine/` (NOT to be committed to git; add `_quarantine/` to the program's local `.gitignore` if not already present).
...
At `~/.agents/.dev/ai/deep-review/2026-05-23/<module>/manifest.json`:
...
```json
{
  ...
  "ledger_jsonl_path": "~/.agents/.dev/ai/deep-review/2026-05-23/FINDINGS-LEDGER.jsonl",
...
`~/.agents/.dev/ai/deep-review/2026-05-23/00-holistic/cross-project-synthesis.md`
...
- **Social module specifically: do not modify, copy, or propose changes to WO-628 artifacts.** The packet at `~/work/peermesh/repo/peermesh-social/.dev/ai/handoffs/2026-05-23-00-55-26Z-wo628-dns-txt-plc-handle-cutover-packet.md` is a parked operator-lane deliverable. Reviewer MAY note where the WO-628 decision shapes risk in adjacent code without arguing the decision itself. Reviewer MAY NOT recommend reversing or amending the decision; the architecture decisions in `ATPROTO-HANDLE-AND-DID-DECISION-2026-05-21.md` are owner-mandated.
...
Read the digest end-to-end (`~/.agents/prompts/security/deep-review-framework-digest.md`). Then read this framework end-to-end. Internalize the rubrics. Note the high-risk surfaces for the assigned module.
...
1. The GAS Priority Board — `~/.agents/data/priority-board/priorities.json` (or `~/.agents/tools/priority-board/bin/gas-priorities --json`) — placement in portfolio. Owner-write only; read it, never write to it.
...
The companion digest at `~/.agents/prompts/security/deep-review-framework-digest.md` is the working manual; this full framework is the contract. Module agents are expected to read the digest first and reach for this full framework when the digest references a section. The digest changes when this framework changes; both are versioned together.

Framework version 2.0 (2026-05-23) integrates external critiques from independent ChatGPT and Claude reviewers (verdicts: both REVISE BEFORE SHIPPING). The changelog at `~/.agents/.dev/ai/deep-review/2026-05-23/framework-v2-changelog.md` enumerates per-change rationale and critique attribution.
