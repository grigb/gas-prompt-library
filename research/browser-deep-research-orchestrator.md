...
6. Verify the signed-in provider route before opening provider pages: GAS
   ...
   profile `~/browser-auth/profiles/browser-research` + encrypted
   auth state under `~/browser-auth/<provider>.json.enc`.
...
- `Claude Browser Deep Research run packet` means one run directory under
  `~/.agents/.dev/ai/deep-research/browser-runs/{run_id}/`.
...
4. Use the verified Interaction Recipes + GAS `agent-browser` route with real
   Google Chrome and `~/browser-auth/profiles/browser-research`.
...
- Every completed provider run packet passes
  `~/.agents/.dev/ai/deep-research/bin/validate-run-packet.py`
