...
`~/.agents/prompts/general/close-steward.md` (sections `### 0` and
`### 9. Current-Lane Retirement And Sibling-Lane Preservation`), the supervisor
preflight `~/.agents/prompts/general/close-supervisor.md` (section
...
`~/.agents/docs/overviews/MASTER-STEWARD-VARIANT.md`. This section
...
`~/.agents-private/project-steward/master-steward/` and the global
...
```bash
~/.agents/tools/gas-method/gas-method threads.list \
...
Use `~/.agents/docs/SESSION-RECOVERY-GUIDE.md` for discovery detail
...
- a referenced path under `~/.agents-private/project-steward/master-steward/`
...
- execute `~/.agents/prompts/general/close-steward.md` first as an internal steward subroutine
...
- execute `~/.agents/prompts/general/close-supervisor.md` first as an internal supervisor preflight
...
This matters most in `FORWARD` (a next step built on an `[inferred]` premise is a guess handed to the next agent as an instruction) and in `STATE -> Key Decisions and Rationale` (a `[relayed:]` decision is a decision someone told you about, not one you can prove was made). Under `~/.agents/docs/protocols/universal-harness-relay-protocol.md` `## Receiver Trust Boundary`, a `[relayed:]` claim can never serve as authority for an irreversible action or as proof that a gate cleared.
...
- Steward session close still uses this prompt; `~/.agents/prompts/general/close-steward.md` is an internal subroutine, not a separate owner-facing routine closeout prompt.
- Blocker Supervisor/Supe session close still uses this prompt; `~/.agents/prompts/general/close-supervisor.md` is an internal preflight, not a separate owner-facing routine closeout prompt.
...
- active `role`
- active `mode`
- source closeout prompt path: `~/.agents/prompts/creation/CREATE-SESSION-RECORD.md`
...
supervisor_subroutine_path: ~/.agents/prompts/general/close-supervisor.md
```
...
- Source closeout prompt: `~/.agents/prompts/creation/CREATE-SESSION-RECORD.md`
- Steward subroutine: `[absolute path to ~/.agents/prompts/general/close-steward.md if used, otherwise None]`
- Role/mode: `[role] / [mode]`
...
- Routing status prompt: `~/.agents/prompts/triage/agent-status-update-for-routing.md`
- Relay protocol: `~/.agents/docs/protocols/universal-harness-relay-protocol.md`
...
target is `~/.agents/.dev/ai/PROJECT-STATUS.md`:
...
`~/.agents/docs/protocols/woq-role-lifecycle.md`, read the live target,
...
`~/.agents/.dev/ai/orchestration/open-codex-agents.md` immediately
...
1. Read `~/.agents/prompts/triage/agent-status-update-for-routing.md`.
...
4. Read `~/.agents/docs/protocols/universal-harness-relay-protocol.md`.
...
- steward sessions ran `~/.agents/prompts/general/close-steward.md` first and recorded that path, or no steward signal was present
...
- `~/.agents/prompts/triage/agent-status-update-for-routing.md` was read and the routing status update was produced
- append-only routing status reports were written when file writes were available, or exact write limitations were recorded
- `~/.agents/docs/protocols/universal-harness-relay-protocol.md` was read before relay selection
